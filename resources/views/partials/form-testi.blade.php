{{-- 
  ...
  1. DI CONTROLLER (TestimonialController@store):
     public function store(Request $request) {
       ...
       Testimonial::create([
         ...
         'user_id' => $user->id,
         'nama' => $user->name, // atau $user->nama sesuai struktur tabel user
         'role' => $user->role ?? 'Mahasiswa', // sesuaikan dengan struktur tabel user
       ]);
  ...
  3. GANTI DATA STATIC dengan DATA dari USER yang LOGIN:
     - Nama: {{ Auth::user()->name ?? 'Guest' }}
     - Role: {{ Auth::user()->role ?? 'Mahasiswa' }}
     - Atau sesuaikan dengan struktur tabel user Anda
--}}
